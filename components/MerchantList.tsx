    ...
    <div className="space-y-8 pb-32">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 text-white">
        ...
        <div className="flex flex-col md:flex-row w-full xl:w-auto gap-4 items-center">
          <div className="bg-slate-900 border-2 border-black rounded-2xl p-1 flex overflow-x-auto custom-scrollbar w-full max-w-full md:w-auto">
            {(['ALL', 'NO_PAYMENTS', 'IN_PROGRESS', 'LIQUIDATED', 'NO_INE'] as FilterType[]).map(f => (
              <button 
                key={f} 
                onClick={() => setActiveFilter(f)} 
                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap flex-shrink-0 ${activeFilter === f ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-white'}`}
              >
                {FILTER_LABELS[f]}
              </button>
            ))}
