...
import { Settings as SettingsIcon, Save, Loader2, Image as ImageIcon, CheckCircle2, AlertCircle, DollarSign, Download, FileSpreadsheet, ShieldCheck } from 'lucide-react';
    ...
    try {
      ...
      const rows = merchants.map(m => {
        ...
        return [
          ...
          m.balance,
          statusLabel,
          `"${zones}"`, // Envolver en comillas para evitar conflictos con comas
      ...
      const blob = new Blob(["\ufeff" + csvContent], { type: 'text/csv;charset=utf-8;' }); // \ufeff para soporte Excel UTF-8
    ...
    <div className="space-y-12 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-8">
          ...
          {/* PANEL DE AUDITORÍA Y RESPALDO (EXPORTACIÓN EXCEL/CSV) */}
          <div className="bg-slate-800 border-4 border-black p-8 rounded-[2.5rem] neobrutalism-shadow-lg flex flex-col gap-6">
             <div className="flex items-center gap-4">
                ...
                <div>
                   <h3 className="text-xl font-black uppercase">Auditoría y Respaldo</h3>
                   <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Descarga de base de datos completa</p>
                </div>
             </div>
             ...
             <div className="p-6 bg-slate-900/50 border-2 border-dashed border-slate-700 rounded-[2rem] flex flex-col gap-4">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-relaxed text-center">
                  Descarga un archivo compatible con <span className="text-emerald-500">Excel</span> con el censo completo, estatus financieros y zonas territoriales.
                </p>
                <button 
        ...
        <div className="bg-slate-900 border-2 border-slate-800 p-8 rounded-[2.5rem] flex flex-col justify-center items-center text-center gap-6 lg:h-fit">
           ...
           <div className="mt-8 p-6 bg-slate-800/50 border-2 border-slate-700 rounded-3xl w-full">
              ...
              <p className="text-xs font-black text-slate-500 uppercase italic">Registro de actividad local: {new Date().toLocaleDateString()}</p>
           </div>
        </div>
      </div>
    </div>
